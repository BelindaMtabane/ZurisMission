...
/// Phase 1 of MainGame: movement tutorial plus tips for cactus water, heat, snakes, and logs.
...

{
    ...
    const float TutorialEndProgress = Level1Config.Phase1End;

    const float JumpHintProgress = 0.12f;

    const float LogHintProgress = Level1Config.RollingLogIntroProgress;

    const float SnakeWaveProgress = Level1Config.SnakeIntroProgress;

    const float HeatHintProgress = Level1Config.HeatWaveStartProgress;
...

    {
        ...
        new TutorialTip(0.05f, "Pass through cactus to refill water and your bucket!"),
...

    {
        ...
        bool showLanes = progress < 0.10f || progress >= SnakeWaveProgress - 0.02f;
